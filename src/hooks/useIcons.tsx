import Angular from "../content/icons/angular.svg";
import Arduino from "../content/icons/arduino.svg";
import AWS from "../content/icons/aws.svg";
import Bash from "../content/icons/bash.svg";
import GCloud from "../content/icons/gcloud.svg";
import Git from "../content/icons/git.svg";
import Hadoop from "../content/icons/hadoop.svg";
import Java from "../content/icons/java.svg";
import JavaScript from "../content/icons/javascript.svg";
import Kubernetes from "../content/icons/kubernetes.svg";
import MangoDB from "../content/icons/mongodb.svg";
import MYSql from "../content/icons/mysql.svg";
import NodeJS from "../content/icons/nodejs.svg";
import Python from "../content/icons/python.svg";
import ReactLogo from "../content/icons/react.svg";
import Spark from "../content/icons/spark.svg";
import Typescript from "../content/icons/typescript.svg";
import CPP from "../content/icons/cpp.svg";
import TensorFlow from "../content/icons/tensorflow.svg";
import Gatsby from "../content/icons/gatsby.svg";
import Docker from '../content/icons/docker.svg';
import Spring from '../content/icons/spring.svg';
import KotlinLang from '../content/icons/kotlinlang.svg';
import Ansible from '../content/icons/ansible.svg';
import Bootstrap from '../content/icons/bootstrap.svg';
import Cassandra from '../content/icons/cassandra.svg';
import HTML from '../content/icons/html.svg';
import Jenkins from '../content/icons/jenkins.svg';
import Kafka from '../content/icons/kafka.svg';
import Keras from '../content/icons/keras.svg';
import NeuralNetwork from '../content/icons/neuralnetwork.svg';
import CSS from '../content/icons/css.svg';
import Postgresql from '../content/icons/postgresql.svg';
import Azure from '../content/icons/azure.svg';
import Oracle from '../content/icons/oracle.svg';
import GoLang from '../content/icons/golang.svg';
import GoGin from "../content/icons/gin.svg";
import NestJs from "../content/icons/nestjs.svg";
import Graphql from "../content/icons/graphql.svg";
import Redis from "../content/icons/redis.svg";
import BitBucket from "../content/icons/bitbucket.svg";
import BlazeMeter from "../content/icons/blazemeter.svg";
import Confluence from "../content/icons/confluence.svg";
import Discord from "../content/icons/discord.svg";
import ElasticSearch from "../content/icons/elasticsearch.svg";
import Grafana from "../content/icons/grafana.svg";
import Idea from "../content/icons/idea.svg";
import Insomnia from "../content/icons/insomnia.svg";
import Jira from "../content/icons/jira.svg";
import Jmeter from "../content/icons/jmeter.svg";
import Kanban from "../content/icons/kanban.svg";
import Kibana from "../content/icons/kibana.svg";
import Postman from "../content/icons/postman.svg";
import Prometheus from "../content/icons/prometheus.svg";
import Pycharm from "../content/icons/pycharm.svg";
import Scrum from "../content/icons/scrum.svg";
import Splunk from "../content/icons/splunk.svg";
import VsCode from "../content/icons/vscode.svg";
import WaterFall from "../content/icons/waterfall.svg";
import WebStorm from "../content/icons/webstorm.svg";
import GoLand from "../content/icons/goland.svg";
import Slack from "../content/icons/slack.svg";
import Firebase from "../content/icons/firebase.svg";
import {Icon} from "../types";
import {IconNames} from "../iconNames";
import GithubActions from "../content/icons/githubactions.svg";
import PWA from "../content/icons/pwa.svg";
import Anthropic from "../content/icons/anthropic.svg";
import LangChain from "../content/icons/langchain.svg";
import HuggingFace from "../content/icons/huggingface.svg";
import Ollama from "../content/icons/ollama.svg";
import N8N from "../content/icons/n8n.svg";
import ComfyUI from "../content/icons/comfyui.svg";
export const icons: Icon[] = [
    { name: IconNames.Angular, icon: Angular },
    { name: IconNames.Arduino, icon: Arduino },
    { name: IconNames.AWS, icon: AWS },
    { name: IconNames.Azure, icon: Azure },
    { name: IconNames.Bash, icon: Bash },
    { name: IconNames.CPP, icon: CPP },
    { name: IconNames.Gatsby, icon: Gatsby },
    { name: IconNames.GoogleCloud, icon: GCloud },
    { name: IconNames.Git, icon: Git },
    { name: IconNames.ApacheHadoop, icon: Hadoop },
    { name: IconNames.Java, icon: Java },
    { name: IconNames.Javascript, icon: JavaScript },
    { name: IconNames.Kotlin, icon: KotlinLang },
    { name: IconNames.Kubernetes, icon: Kubernetes },
    { name: IconNames.MongoDB, icon: MangoDB },
    { name: IconNames.MySQL, icon: MYSql },
    { name: IconNames.NodeJS, icon: NodeJS },
    { name: IconNames.Oracle, icon: Oracle },
    { name: IconNames.Python, icon: Python },
    { name: IconNames.Postgresql, icon: Postgresql },
    { name: IconNames.React, icon: ReactLogo },
    { name: IconNames.ApacheSpark, icon: Spark },
    { name: IconNames.SpringBoot, icon: Spring },
    { name: IconNames.TensorFlow, icon: TensorFlow },
    { name: IconNames.Typescript, icon: Typescript },
    { name: IconNames.Docker, icon: Docker },
    { name: IconNames.Ansible, icon: Ansible },
    { name: IconNames.Bootstrap, icon: Bootstrap },
    { name: IconNames.ApacheCassandra, icon: Cassandra },
    { name: IconNames.HTML, icon: HTML },
    { name: IconNames.Jenkins, icon: Jenkins },
    { name: IconNames.ApacheKafka, icon: Kafka },
    { name: IconNames.Keras, icon: Keras },
    { name: IconNames.NeuralNetworks, icon: NeuralNetwork },
    { name: IconNames.CSS, icon: CSS },
    { name: IconNames.GoLang, icon: GoLang },
    { name: IconNames.GoGin, icon: GoGin },
    { name: IconNames.NestJs, icon: NestJs },
    { name: IconNames.GraphQL, icon: Graphql },
    { name: IconNames.Redis, icon: Redis },
    { name: IconNames.BitBucket, icon: BitBucket },
    { name: IconNames.BlazeMeter, icon: BlazeMeter },
    { name: IconNames.Confluence, icon: Confluence },
    { name: IconNames.Discord, icon: Discord },
    { name: IconNames.ElasticSearch, icon: ElasticSearch },
    { name: IconNames.Grafana, icon: Grafana },
    { name: IconNames.Idea, icon: Idea },
    { name: IconNames.Insomnia, icon: Insomnia },
    { name: IconNames.Jira, icon: Jira },
    { name: IconNames.Jmeter, icon: Jmeter },
    { name: IconNames.Kanban, icon: Kanban },
    { name: IconNames.Kibana, icon: Kibana },
    { name: IconNames.Postman, icon: Postman },
    { name: IconNames.Prometheus, icon: Prometheus },
    { name: IconNames.Pycharm, icon: Pycharm },
    { name: IconNames.Scrum, icon: Scrum },
    { name: IconNames.Agile, icon: Scrum },
    { name: IconNames.Splunk, icon: Splunk },
    { name: IconNames.VsCode, icon: VsCode },
    { name: IconNames.WaterFall, icon: WaterFall },
    { name: IconNames.WebStorm, icon: WebStorm },
    { name: IconNames.GoLand, icon: GoLand },
    { name: IconNames.Slack, icon: Slack },
    { name: IconNames.Firebase, icon: Firebase },
    {name: IconNames.GithubActions, icon: GithubActions},
    {name: IconNames.PWA, icon: PWA},
    {name: IconNames.Anthropic, icon: Anthropic},
    {name: IconNames.LangChain, icon: LangChain},
    {name: IconNames.HuggingFace, icon: HuggingFace},
    {name: IconNames.Ollama, icon: Ollama},
    {name: IconNames.N8N, icon: N8N},
    {name: IconNames.ComfyUI, icon: ComfyUI},
].sort((a, b) => a.name.localeCompare(b.name));
